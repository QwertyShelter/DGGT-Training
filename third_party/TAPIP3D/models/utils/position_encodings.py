...
class RotaryPositionEncoding4D(RotaryPositionEncoding):
    ...
    def __init__(self, feature_dim, pe_type='Rotary4D'):
        super().__init__(feature_dim, pe_type)
    ...
    def forward(self, XYZT):
        ...
        @return:
        '''
        bsize, npoint, _ = XYZT.shape
        x_position, y_position, z_position, t_position = (
            XYZT[..., 0:1], XYZT[..., 1:2], XYZT[..., 2:3], XYZT[..., 3:4]
        )
        div_term = torch.exp(
            torch.arange(0, self.feature_dim // 4, 2, dtype=torch.float, device=XYZT.device)
        ...
        sinx, cosx, siny, cosy, sinz, cosz, sint, cost = map(
            lambda feat: torch.stack([feat, feat], -1).view(bsize, npoint, -1),
            [sinx, cosx, siny, cosy, sinz, cosz, sint, cost]
        )

        position_code = torch.stack([
            torch.cat([cosx, cosy, cosz, cost], dim=-1),  # cos_pos
            torch.cat([sinx, siny, sinz, sint], dim=-1)  # sin_pos
        ], dim=-1)
