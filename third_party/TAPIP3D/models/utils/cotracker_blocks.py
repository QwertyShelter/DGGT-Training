...
def test_attention_modules():
    ...
    x = torch.randn(batch_size, seq_len1, query_dim)
    context = torch.randn(batch_size, seq_len2, query_dim)

    attn_bias = torch.randn(batch_size, num_heads, seq_len1, seq_len2)

    attention1 = AttentionOrig(query_dim, num_heads=num_heads, dim_head=dim_head, qkv_bias=qkv_bias)
    attention2 = Attention(query_dim, num_heads=num_heads, dim_head=dim_head, qkv_bias=qkv_bias)

    attention2.to_q.weight.data.copy_(attention1.to_q.weight.data)
    attention2.to_q.bias.data.copy_(attention1.to_q.bias.data)
    attention2.to_kv.weight.data.copy_(attention1.to_kv.weight.data)
    attention2.to_kv.bias.data.copy_(attention1.to_kv.bias.data)
    attention2.to_out.weight.data.copy_(attention1.to_out.weight.data)
    attention2.to_out.bias.data.copy_(attention1.to_out.bias.data)

    output1 = attention1(x, context=context, attn_bias=attn_bias)
    output2 = attention2(x, context=context, attn_bias=attn_bias)

    max_diff = (output1 - output2).abs().max().item()
    print("Max absolute difference between outputs:", max_diff)
