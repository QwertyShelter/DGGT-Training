    ...
    def forward(self, query, value, diff_ts=None,
        ...
        for i in range(self.num_layers):
            query = self.attn_layers[i](
                query, value, diff_ts, query_pos, value_pos, pad_mask=pad_mask,
            )
...
def build_mlp(
    ...
    last_act_layer: Callable[[], nn.Module] = nn.Identity,
    input_norm: Callable[[int], nn.Module] = nn.LayerNorm,
    output_norm: Callable[[int], nn.Module] = nn.Identity,
    residual: bool = False,
) -> nn.Module:
    layers: List[nn.Module] = [input_norm(input_size)] if input_norm is not None else []
    cur_size = input_size
    ...
    layers.append(last_act_layer())
    layers.append(output_norm(output_size))

    if not residual:
        return nn.Sequential(*layers)
    else:
        return ResidualLayer(nn.Sequential(*layers))
